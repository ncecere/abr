...
- **Automation loop**:
  ...
  2. `SEARCH_BOOK` queries every enabled indexer via the Newznab client, tries strict queries first (`m4b -part -disc -cd`), rejects multi-part or undersized releases, and stores the best match in `releases` before scheduling `GRAB_RELEASE`. Indexer failures are logged individually so a single bad provider never aborts the search.
  ...
  5. `IMPORT_DOWNLOAD` selects the best audiobook file, moves it into `libraryRoot/audiobook/{author}/{title}`, records the file metadata, marks the book `AVAILABLE`, and emits the final activity events. When a download only contains chapter-style multi-file releases, the importer schedules a `MERGE_TRACKS` job that runs `ffmpeg` to concatenate the tracks; once the merged container exists the import is retried automatically. If merging fails, the job logs the failure, reverts the book to `MISSING`, and requeues a search.
