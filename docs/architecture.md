...
- **Automation loop**:
  ...
  5. `IMPORT_DOWNLOAD` applies any download-client path mappings, scans the resolved directory for track files, and only touches the library once the scan succeeds. Single-file releases move into `libraryRoot/audiobook/{author}/{title}` and are recorded in `book_files`. If multiple tracks with the same extension exist, ABR schedules a `MERGE_TRACKS` job (ffmpeg concat) before retrying import. Merge failures are logged, the book reverts to `MISSING`, and a new search is enqueued.
