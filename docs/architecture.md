...
- **Automation loop**:
  1. User adds a book through the `/api/books` POST handler (or UI). Book is persisted with state `MISSING` and a `SEARCH_BOOK` job is enqueued.
...
- `libraryRoot` is always ensured on boot and when edits land in the settings API.
