...
| `formats` | Enabled ebook formats + priority order. | `name`, `extensions` (JSON), `priority`, `enabled` |
...
| `books` | Core library records. | `openlibrary_work_id`, `title`, `authors_json`, `state` (`MISSING`/`AVAILABLE`), `cover_url`, `cover_path`, timestamps |
